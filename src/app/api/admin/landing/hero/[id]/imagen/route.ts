...
import { updateHeroSlide, uploadHeroImage } from "@/features/admin/services/landing-admin-service";
...
export async function POST(request: Request, context: { params: Promise<Params> }) {
  try {
    ...
    return NextResponse.json(
      { error: "internal_server_error", message: "Could not upload hero image." },
      { status: 500 },
    );
  }
}

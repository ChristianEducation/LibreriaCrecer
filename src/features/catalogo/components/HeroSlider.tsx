...
import type {
  ...
  HeroSlideViewModel,
  HeroViewModel,
} from "../view-models/hero-view-model";
...
function buildOverlayStyle(variant: HeroOverlayVariantViewModel, opacityPercent: number): CSSProperties | null {
  if (variant === "none") {
    return null;
  }
  ...
  return {
    background:
      "linear-gradient(to bottom, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.55) 50%, rgba(0,0,0,0.95) 100%)",
    opacity,
  };
}
...
export function HeroSlider({ data }: HeroSliderProps) {
  ...
  const activeSlide: HeroSlideViewModel = items[activeIndex] ?? items[0];
